...
→ **4-Layer Analysis 자동 실행**
   > 다음 명령어를 실행하여 분석 결과를 얻으세요:
   > `python ~/.gemini/antigravity/skills/translation-specialist/linguistic_analysis.py --text "분석할 텍스트"`
   > 또는 파일이 있는 경우:
   > `python ~/.gemini/antigravity/skills/translation-specialist/linguistic_analysis.py <파일경로>`
