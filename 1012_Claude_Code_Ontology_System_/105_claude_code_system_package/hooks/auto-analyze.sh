...
# 기존 분석 로직(1~137줄)에는 손대지 않음
# 상주 서버(18765)에 curl 요청 — 실패 시 기존 분석만 출력
# =============================================
if [ ${#PROMPT} -ge 10 ] && [ "$SKIP_MEMORY_RECALL" != "1" ]; then
    # URL 인코딩 (jq 활용)
    ENCODED_PROMPT=$(echo -n "$PROMPT" | jq -sRr @uri 2>/dev/null)
    ...
    if [ -n "$ENCODED_PROMPT" ]; then
        ...
        # 결과가 비어있지 않고 에러가 아닌 경우
        if [ -n "$MEMORY_RESULTS" ] && [ "$MEMORY_RESULTS" != "[]" ] && ! echo "$MEMORY_RESULTS" | grep -q '"error"'; then
            MEMORY_CONTEXT=$(echo "$MEMORY_RESULTS" | python3 -c "
import json, sys
try:
    ...
    if isinstance(results, list) and len(results) > 0:
        ...
        for r in results:
            ...
            lines.append(f'- [{score_pct}%] {mem_id}: {summary}')
        print('\n'.join(lines))
except:
    pass
" 2>/dev/null)
...
# C5 Observability: HOOK_RECOMMEND 이벤트 로그
if [ -n "$ANALYSIS" ]; then
    HOOK_REC_CHAIN=$(echo "$ANALYSIS" | grep -oP '권장 체인: \K\S+' | head -1)
    if [ -n "$HOOK_REC_CHAIN" ]; then
        echo "$(date '+%Y-%m-%d %H:%M') | HOOK_RECOMMEND | ${HOOK_REC_CHAIN} | -" \
            >> "$HOME/.claude/logs/$(date +%Y%m%d).log"
    fi
fi
