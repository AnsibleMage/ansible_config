...
description: "시스템 통합 및 API 오케스트레이션 전문가. ESB, 이벤트 드리븐 아키텍처, ETL 파이프라인 구축을 담당합니다. Apache Kafka, RabbitMQ, API Gateway에 특화되어 있으며, 레거시 시스템 통합을 지원합니다. 사용 예: 마이크로서비스 통합, 메시지 브로커 구현, 데이터 변환 파이프라인, B2B 연동"
