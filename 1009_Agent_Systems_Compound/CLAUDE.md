# CLAUDE.md - Claude Code Integrated Guidelines V3.6

> Version: 3.6 | Updated: 2026-02-04
> Based on: V3.5 + 응답 완료 프로토콜 + prompt_analyzer V2.1
...
### UserPromptSubmit Hook (🆕 V3.5)

> **모든 프롬프트 입력 시 자동 실행** - Claude가 처리하기 전에 4-Layer 분석
...
### F. DocChain (문서)
```
문서 유형 식별 → /docx[-] | /pdf[-] | /pptx[-] | /xlsx[-] → [optional] quality_reviewer[S]
```

### G. DesignChain (디자인)
```
[optional] /brand-guidelines[-] → (/canvas-design[-] ∥ /theme-factory[-]) | /frontend-design[-]
```
...
```
(WebSearch[∥] ∥ Context7/MCP[∥]) → multidimensional_analyst[O] → Write[-]
```
> **Use Case**: 기술 분석, 적합성 조사, 트렌드 연구
> **특징**: 외부 정보 병렬 수집 → 다차원 분석 → 문서화
...
*Claude Code Integrated Guidelines V3.6 - 지침 기반 메모리 저장 + prompt_analyzer V2.1*
